// Compile with just `gcc main.c -o main`
// To compile a Forth program, run `./main file.f; gcc output.s output`

#include <stdio.h>
#include <stdlib.h>
#include <memory.h>

#define LINKED_LIST_EASY_NAMES
#include "linked_list.c"

#include "forth_words_builtin.c"
...
// Compare string_t to a char array
bool string_is(string_t str1, char* str2){
	for(size_t i = str1.len ; i > 0; i--){
		if(!str2[i - 1]) return 0;
		if(str2[i - 1] != str1.data[i - 1]) return 0;
	}
	return 1;
}
...
string_t* strings_join(string_t* s_arr[]){
	...
	string_t* out = malloc(sizeof(total_size));
	out->len = total_size;
	out->data = malloc(out->len);
...
// Converts byte string to hex string ("hash")
string_t* get_word_name_hash(string_t* word_name){
	...
	for(size_t byte_i = 0; byte_i < word_name->len; byte_i++){
		char byte = word_name->data[byte_i];
		*ptr = hexmap[byte >> 4];
		ptr += 1;
		*ptr = hexmap[byte & 0b1111];
		ptr += 1;
	}
...
ast_node_t* get_next_ast_node(char** pgm){
	...
	if(string_is(word, ":")){
		...
		output_node->data.word_def = malloc(sizeof(struct __ast_node_word_def_data));
		
		output_node->data.word_def->word_name = string_lowercase(string_copy(&word_def_name));
...
void get_ast_node_asm_code(ast_node_t* node, linked_list_t* out, compilation_data_t* comp_data){
	switch(node->type){
...
void preprocess_ast(ast_node_t* ast_node){
	...
	switch(ast_node->type){
		...
		case AST_TYPE_WORD_DEF:
			...
			break;
		case AST_TYPE_NODE_LIST:
			linked_list_node_t* llist_node = ast_node->data.ast_node_list->start_node;
			while(llist_node){
...
#define TABS(n) for(size_t i = 0; i < n; i++) printf("\t")
...
linked_list_t* get_builtin_forth_word_def(){
	...
	size_t n_words = sizeof(FORTH_WORDS_BUILTIN) / sizeof(FORTH_WORDS_BUILTIN[0]);
	...
	for(size_t i = 0; i < n_words; i++){
		...
		def_node->data.word_def = malloc(sizeof(struct __ast_node_word_def_data));
...
int main(int argc, char **argv){
	...
	init_runtime_node->data.inline_asm = string_create(FORTH_RUNTIME_START_CODE);
